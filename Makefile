...
THEME_DIR=theme/taydellinen
SYLIUS_VERSION=1.11.0
SYMFONY=cd ${APP_DIR} && symfony
COMPOSER=symfony composer
CONSOLE=${SYMFONY} console
export COMPOSE_PROJECT_NAME=taydellinen
...
define yarn
	cd ${APP_DIR} && ${COMPOSE} run --rm --user node node sh -c "cd ${THEME_DIR}; yarn $(1)"
endef
...
setup_application:
	...
	(cd ${APP_DIR} && ${COMPOSER} config extra.symfony.require '>=4.4 <5.4')
...
docker.up: ## Start the docker containers
	cd ${APP_DIR} && ${COMPOSE} up -d --remove-orphans --build
.PHONY: docker.up
...
yarn.bash: ## Start a bash in in the Yarn container
	cd ${APP_DIR} && ${COMPOSE}  run --rm --user node node sh
...
yarn.add:
	cd ${APP_DIR} && ${COMPOSE} run --rm --user node node sh -c "yarn add ${ARGS}"

yarn.install:
	cd ${APP_DIR} && ${COMPOSE} run --rm --user node node sh -c "yarn install"

yarn.build:
	cd ${APP_DIR} && ${COMPOSE} run --rm --user node node sh -c "yarn build"

yarn.watch:
	cd ${APP_DIR} && ${COMPOSE} run --rm --user node node sh -c "yarn watch"
